...
async function apply(ctx, _config) {
  ctx.tools.register(defineTool({
    name: "element_info",
    description: "返回碳元素（Carbon）完整资料：名称、元素符号、原子序数、相对原子质量、类别、族、周期、常温状态、电子构型。",
    parameters: {},
    ...
    execute: async (args) => ((a) => ({ output: {"name":"Carbon（碳）","symbol":"C","atomic_number":6,"atomic_mass":12.011,"category":"非金属","group":14,"period":2,"phase":"固体","electron_config":"[He] 2s2 2p2"} }))(args),
  }));
  ctx.tools.register(defineTool({
    name: "element_field",
    description: "按字段名查询碳元素（Carbon）的单项资料（可用于流程取数）。",
    ...
    execute: async (args) => ((a) => { const row = {"name":"Carbon（碳）","symbol":"C","atomic_number":6,"atomic_mass":12.011,"category":"非金属","group":14,"period":2,"phase":"固体","electron_config":"[He] 2s2 2p2"}; const labels = {"name":"名称","symbol":"元素符号","atomic_number":"原子序数","atomic_mass":"相对原子质量","category":"类别","group":"族","period":"周期","phase":"常温状态","electron_config":"电子构型"}; if (!(a.field in row)) return { output: null }; return { output: { field: a.field, label: labels[a.field], value: row[a.field] } }; })(args),
  }));
}
